...
const parseColor = (color: string): RGBA => {
  if (color.startsWith('rgba')) {
    const parts = color.substring(5, color.length - 1).split(',').map(s => parseFloat(s.trim()));
...
export const ColorPicker: React.FC<ColorPickerProps> = ({ color, onChange, debounceMs = 100 }) => {
  ...
  return (startEvent: React.MouseEvent | React.TouchEvent) => {
    ...
    const onMove = (e: MouseEvent | TouchEvent) => handleDrag(e, ref, updater);
    const onEnd = () => {
      interactionRef.current = false;
  ...
  const handleSaturationChange = ({ x, y }: { x: number, y: number }, rect: DOMRect) => {
    const s = (x / rect.width) * 100;
    const v = (1 - y / rect.height) * 100;
    setHsva(prev => {
      if (Math.round(prev.s) === Math.round(s) && Math.round(prev.v) === Math.round(v)) return prev;
      return { ...prev, s, v };
    });
  };
  ...
  const handleHueChange = ({ x }: { x: number }, rect: DOMRect) => {
    ...
    setHsva(prev => (Math.round(prev.h) === Math.round(h) ? prev : { ...prev, h }));
  };

  const handleAlphaChange = ({ x }: { x: number }, rect: DOMRect) => {
    const a = x / rect.width;
    setHsva(prev => (Math.round(prev.a * 100) === Math.round(a * 100) ? prev : { ...prev, a }));
  };
  ...
  const handleRgbaChange = (key: keyof RGBA, value: string) => {
    ...
    next[key] = key === 'a' ? Math.max(0, Math.min(1, num)) : Math.max(0, Math.min(255, Math.round(num)));
  ...
  return (
    <div className="color-picker" onMouseDown={e => e.stopPropagation()}>
      <div
        ...
        ref={saturationRef}
        onMouseDown={createDragHandler(saturationRef, handleSaturationChange)}
        onTouchStart={createDragHandler(saturationRef, handleSaturationChange)}
      >
