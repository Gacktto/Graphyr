...
export default function Rightbar() {
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (
                ...
                if (!(event.target as HTMLElement).closest('.color-control-clickable')) {
                     setPickerState(null);
                }
            }
        };
    ...
    useLayoutEffect(() => {
        if (pickerState && pickerRef.current && menuRef.current) {
            ...
            let finalLeft = triggerRect.left - pickerRect.width - 10;

            if (finalLeft < 0) {
                finalLeft = triggerRect.right + 10;
            }
             if (finalTop + pickerRect.height > window.innerHeight) {
                finalTop = window.innerHeight - pickerRect.height - 10;
            }
