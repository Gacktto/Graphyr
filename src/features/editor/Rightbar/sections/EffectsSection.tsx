...
export const EffectsSection: React.FC<EffectsSectionProps> = ({
    ...
    const availableEffects = useMemo(
        () =>
            (Object.keys(effectDefaults) as EffectType[]).filter(
                (e) => !activeEffects.includes(e)
            ),
        [activeEffects]
    );
    ...
    const updateFilter = (
        effects: EffectType[],
        values: Record<EffectType, string>
    ) => {
        const sortedEffects = [...effects];

        const dropShadowIndex = sortedEffects.indexOf('drop-shadow');

        if (dropShadowIndex > -1) {
            sortedEffects.splice(dropShadowIndex, 1);
            sortedEffects.push('drop-shadow');
        }

        const filterString = sortedEffects
            .map((e) => `${e}(${values[e]})`)
            .join(' ');
    ...
    const handleAddEffect = (effect: EffectType) => {
        ...
        if (effect === 'drop-shadow') {
            ...
            newEffectValues['drop-shadow'] =
                `${defaultParts.offsetX} ${defaultParts.offsetY} ${defaultParts.blur} ${defaultParts.color}`;
        }
        setActiveEffects(newActiveEffects);
                                ...
                                <div className={styles.row}>
                                    <div className={styles.groupInput}>
                                        <input
                                            ...
                                            value={dropShadowParts.offsetX}
                                            onChange={(e) =>
                                                handleDropShadowChange(
                                                    'offsetX',
                                                    e.target.value
                                                )
                                            }
